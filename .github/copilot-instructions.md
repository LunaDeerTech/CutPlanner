...
3. 参数配置：锯片厚度、留边距、算法选择等选项，使用表单控件（输入框、开关、下拉等）。
...
2. **设置页面**
   - 页面右上角放一个齿轮图标，点击后弹出设置面板
   - 设置面板包含单位选择（毫米 / 英寸）、默认锯片厚度等配置
   - 保存设置到 localStorage，并在页面加载时读取
...
4. **目标板列表手动输入**  
   - 用户点击添加后弹出表单，输入目标板长、宽、数量
   - 列表显示已添加的目标板，支持删除（不需要编辑按钮）
   - 可在列表中编辑数量、尺寸
   - 校验输入是否为正数
   - 保存到状态管理中
