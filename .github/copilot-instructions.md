...
- 要求工具完全在前端运行，不依赖后端服务。
- 最终用户流程包括：输入原料尺寸 → 下载切割清单模板 → 在模板中填写目标板尺寸与数量 → 上传/导入模板 → 配置切割参数（如锯片厚度等） → 执行排版算法 → 显示切割示意图（含尺寸标注） → 可导出 PNG 图像或报告。
...
所有主要操作都用清晰的按钮（“下一步”“返回”“生成排版”等），避免让用户在多个页面里迷失。
在桌面端显示较宽的排版图区域，移动端自动切换为分步卡片布局。
...
├── pages/               # 业务页面（RawInput、UploadList、ConfigParams、CuttingResult）
├── store/               # Pinia 状态管理
│   ├── material.ts      # 原料状态
│   ├── settings.ts      # 配置项状态
│   └── cutting.ts       # 排版结果状态
